...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6 p-6">
        <div>
            <?php ob_start(); ?><div class="font-medium [:where(&amp;)]:text-zinc-800 [:where(&amp;)]:dark:text-white text-2xl [&amp;:has(+[data-flux-subheading])]:mb-2 [[data-flux-subheading]+&amp;]:mt-2" data-flux-heading><?php ob_start(); ?>Edit Order<?php echo trim(ob_get_clean()); ?></div>
<?php echo ltrim(ob_get_clean()); ?>
            <?php ob_start(); ?><div class="text-sm [:where(&amp;)]:text-zinc-500 [:where(&amp;)]:dark:text-white/70" data-flux-subheading>
    <?php ob_start(); ?>Perbarui detail informasi pesanan klien #<?php echo e($order->id); ?><?php echo trim(ob_get_clean()); ?>
...
<?php echo ltrim(ob_get_clean()); ?>
    ...
    <?php ob_start(); ?>
            ...
            <form action="<?php echo e(route('orders.update', $order->id)); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                <?php echo csrf_field(); ?>
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <input type="text" name="nama_klien" value="<?php echo e(old('nama_klien', $order->nama_klien)); ?>" placeholder="Masukkan nama lengkap klien..." class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950" required>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Layanan</label>
                        <input type="text" name="layanan" value="<?php echo e(old('layanan', $order->layanan)); ?>" placeholder="Pilih jenis layanan..." class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950" required>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Jumlah</label>
                            <input type="number" name="jumlah" value="<?php echo e(old('jumlah', $order->jumlah)); ?>" class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">Harga Awal</label>
                            <input type="number" name="harga" value="<?php echo e(old('harga', intval($order->harga_awal))); ?>" placeholder="Rp" class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($order->file_referensi): ?>
                            <p class="mt-2 text-xs text-zinc-500">
                                File saat ini: <a href="<?php echo e(asset('storage/' . $order->file_referensi)); ?>" target="_blank" class="text-indigo-600 hover:underline">Lihat File</a>
                            </p>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    </div>
                </div>
...
<?php endif; ?>
<?php /**PATH /home/caplinux/project/joki_web/resources/views/orders/edit.blade.php ENDPATH**/ ?>